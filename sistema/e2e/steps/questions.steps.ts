...
When('I create a new question with description {string}', async function (description: string) {
  await this.page.fill('[data-testid="question-description-input"]', description);
});

When('I add an alternative {string} which is not correct', async function (alternative: string) {
  const inputs = this.page.locator('input[data-testid^="alternative-description-input-"]');
  ...
  await this.page.fill(`[data-testid="alternative-description-input-${targetIndex}"]`, alternative);
  await this.page.uncheck(`[data-testid="alternative-correct-checkbox-${targetIndex}"]`);
});
...
When('I add an alternative {string} which is correct', async function (alternative: string) {
  ...
  await this.page.fill(`[data-testid="alternative-description-input-${targetIndex}"]`, alternative);
  await this.page.check(`[data-testid="alternative-correct-checkbox-${targetIndex}"]`);
});
...
Given('there is an existing question {string}', async function (description: string) {
  ...
  if (await existing.count() === 0) {
    await this.page.fill('[data-testid="question-description-input"]', description);
    await this.page.fill(`[data-testid="alternative-description-input-0"]`, 'Dummy');
    await this.page.click('[data-testid="submit-question-btn"]');
    await expect(this.page.locator(`text=${description}`).first()).toBeVisible();
  }
});
...
When('I update the question description to {string}', async function (newDescription: string) {
  ...
  const editBtn = this.page.locator(`button[aria-label="Edit What is the capital of France?"]`); // But we need the old description!
  ...
  await this.page.fill('[data-testid="question-description-input"]', newDescription);
});
...
When('I delete the question {string}', async function (description: string) {
  ...
  const deleteBtn = this.page.locator(`button[aria-label="Delete ${description}"]`);
