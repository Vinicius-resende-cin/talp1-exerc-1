...
import { Question, Alternative, fetchQuestions, createQuestion, updateQuestion, deleteQuestion } from "./api";
...
export function QuestionsPage() {
  ...
  const handleEdit = (question: Question) => {
    ...
    setAlternatives(question.alternatives.length > 0 ? question.alternatives : [{ description: "", isCorrect: false }]);
  };
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '2rem', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ background: '#f9f9f9', padding: '1.5rem', borderRadius: '8px', marginBottom: '2rem' }}>
        <h2 data-testid="form-title">{editingId ? 'Edit Question' : 'Add New Question'}</h2>
        <form onSubmit={handleSubmit} data-testid="question-form">
          ...
          <div style={{ marginBottom: '1rem' }}>
            <h3 style={{ margin: '0 0 0.5rem 0', fontSize: '1.1rem' }}>Alternatives</h3>
            {alternatives.map((alt, index) => (
              <div key={index} style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', marginBottom: '0.5rem' }} data-testid={`alternative-item-${index}`}>
                <input 
                  type="text"
                ...
                {alternatives.length > 1 && (
                  <button 
                    ...
                    style={{ padding: '0.5rem 1rem', background: '#ffebee', color: '#c62828', border: 'none', borderRadius: '4px', cursor: 'pointer' }}
                  >
                    Remove
                  </button>
                )}
            ...
            <button 
              ...
              style={{ marginTop: '0.5rem', padding: '0.5rem 1rem', background: '#e3f2fd', color: '#1565c0', border: 'none', borderRadius: '4px', cursor: 'pointer' }}
            >
              + Add Alternative
            </button>
          </div>
          ...
          <div style={{ display: 'flex', gap: '1rem' }}>
            <button 
              ...
              style={{ padding: '0.75rem 1.5rem', background: '#2e7d32', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}
            >
            ...
            {editingId && (
              <button 
                ...
                style={{ padding: '0.75rem 1.5rem', background: '#9e9e9e', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}
              >
                Cancel
              </button>
            )}
      ...
      <div>
        ...
        {questions.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }} data-testid="questions-list">
            {questions.map(q => (
              <div key={q.id} data-testid={`question-card-${q.id}`} style={{ border: '1px solid #e0e0e0', padding: '1rem', borderRadius: '8px' }}>
                <h3 style={{ margin: '0 0 0.5rem 0' }}>{q.description}</h3>
                <ul style={{ paddingLeft: '1.5rem', marginBottom: '1rem' }}>
                  {q.alternatives && q.alternatives.map((alt, i) => (
                    <li key={alt.id || i} style={{ fontWeight: alt.isCorrect ? 'bold' : 'normal', color: alt.isCorrect ? '#2e7d32' : 'inherit' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  ...
                  <button 
                    ...
                    style={{ padding: '0.5rem 1rem', background: '#d32f2f', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}
                  >
                    Delete
                  </button>
                </div>
